...
- UI: calling happens via "call nodes" that correspond to a function node. left: parameters are labeled and accept values. right: 0 or 1 outputs.
  ...
  - a browser tab -style switch mechanism at top, which toggles views between either "main" or "runtime" flows (see def of "runtime" below)
- Implementation:
  ...
  - each function def has at most 1 call instance. all call nodes for a given function share the same call instance - races are not handled explicitly, but merely encouraged to be sequential via the "run button" idiom.
  - an invocation happens as follows:
    - if a call instance already exists for its corresponding function def, remove it.
    - create a new call instance:
      - an instance is created by cloning the corresponding func def's bounding box and subgraph and assigning the caller's arg values to the corresponding param nodes.
    ...
    - retrieve the result of the call instance to the sall node, which becomes its value as any normal node would have.
